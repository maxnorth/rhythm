...
This document outlines the design for adding optional Redis backend support to Currant, allowing users to choose between PostgreSQL's durability and Redis's performance on a per-job basis.
...
- **Throughput:** ~200 jobs/sec sustained with 5 workers
- **Latency:** p50=5.2ms, p95=9.4ms, p99=16.7ms
- **Durability:** Full ACID guarantees, survives crashes
- **Optimization journey:** Started at 24 jobs/sec, achieved 8.3x improvement through:
  - LISTEN/NOTIFY for instant notifications
  - Batch claiming (multiple jobs per query)
  - Global connection pool (fixed major bottleneck)
  - Local job queue with prefetching
  - Batch completion (1ms flush interval)

### Competitive Landscape

**Redis-backed queues (Dramatiq, Celery, Huey):**
- Throughput: 1,700-5,500 jobs/sec (10 workers)
- Trade-off: In-memory storage, can lose jobs on crash
- Requires separate Redis service

**PostgreSQL-backed durable queues:**
- **Temporal:** 8-16 workflows/sec (complex workflow engine)
- **Currant:** 200 jobs/sec (10-20x faster than Temporal!)
- **pg_boss:** 100-500 jobs/sec
- **Graphile Worker:** 200-1,000 jobs/sec

### Target Performance with Redis
- **Redis + eventual durability:** 800-1,500 jobs/sec (~4-7x improvement)
- **Redis only (no durability):** 2,000-4,000 jobs/sec (~10-20x improvement)
- **Hybrid mode:** Best of both worlds for different job types

---

## Design Philosophy

### Core Principle: Per-Job Backend Selection

Users should be able to choose the appropriate backend for each job type based on their specific needs:

```python
# Critical financial transaction - full durability
@job(queue="payments", backend="postgres")
...
@job(queue="analytics", backend="redis", durability="eventual")
async def track_event(event: dict):
    await analytics.track(event)

# Ultra high-throughput metrics - ephemeral is fine
@job(queue="metrics", backend="redis", durability="none")
...
3. **Per-job granularity** - Different jobs can use different backends
4. **Transparent to workers** - Workers don't care which backend is used
5. **No code changes to job functions** - Just decorator configuration
...
pub trait QueueBackend: Send + Sync {
    ...
    /// Claim multiple job IDs from the queue
    ...
    /// Remove a job from the queue (after completion/failure)
...
```
- **Pros:** 2,000-4,000+ jobs/sec throughput
- **Cons:** Can lose jobs on Redis crash (durability="none")
...
### Job Decorator Enhancement

```python
from currant import job
...
|------|---------|------------|-------|---------------------|------------|
| `full` | `postgres` | ✓ Immediate | ✗ | ✓ | ~200 jobs/sec |
| `eventual` | `redis` | ✓ Async write | ✓ Queue | ✓ (eventually) | ~800-1,500 jobs/sec |
| `none` | `redis` | ✗ | ✓ Queue only | ✗ | ~2,000-4,000+ jobs/sec |
...
- [ ] Add `backend` parameter to `@job()` decorator
...
**Expected gain:** 10-20x improvement for Redis jobs
...
8. If Redis crashes: jobs can be recovered from PostgreSQL
```
...
- Claim: ZPOPMIN currant:queue:emails:p5 10  (claim 10 jobs)
...
### Integration Tests
- [ ] End-to-end job execution with Redis backend
- [ ] PostgreSQL fallback when Redis unavailable
- [ ] Mixed backend workloads (some jobs Redis, some PostgreSQL)
...
@job(queue="test", backend="postgres")
async def noop_pg(): pass

# Scenario 2: Redis no durability (max speed)
@job(queue="test", backend="redis", durability="none")
async def noop_redis_fast(): pass

# Scenario 3: Redis eventual durability (balanced)
@job(queue="test", backend="redis", durability="eventual")
async def noop_redis_eventual(): pass

# Run benchmarks
currant bench --workers 5 --jobs 1000 --backend postgres
currant bench --workers 5 --jobs 1000 --backend redis --durability none
currant bench --workers 5 --jobs 1000 --backend redis --durability eventual
```
...
3. **Update jobs selectively**: Add `backend="redis"` to high-throughput jobs
4. **No worker changes needed**: Workers auto-detect enabled backends

### Rollback Strategy
- Remove `backend="redis"` from job decorators
- Jobs revert to PostgreSQL automatically
...
- AOF persistence for durability="eventual" jobs

### Capacity Planning
| Workers | Backend | Expected Throughput | Redis Memory (1M jobs) |
|---------|---------|---------------------|------------------------|
| 5 | PostgreSQL | ~200 jobs/sec | - |
| 5 | Redis (none) | ~2,000 jobs/sec | ~50 MB |
| 5 | Redis (eventual) | ~1,000 jobs/sec | ~50 MB |
| 10 | Redis (none) | ~4,000 jobs/sec | ~50 MB |
...
- Jobs being written to PostgreSQL in background
- Some recent jobs may be lost (< 1 second window)
...
|--------------|-----------|---------------|------------|
| PostgreSQL (current) | 200 jobs/sec | 5.2ms | Full ACID |
| Redis + eventual | 800-1,500 jobs/sec | 2-3ms | Async persist |
| Redis only | 2,000-4,000 jobs/sec | 1-2ms | None |
...
### Why per-job backend selection?
- Different jobs have different requirements
- Critical jobs need durability, high-volume jobs need speed
...
- Celery with Redis: 1,700 jobs/sec (10 workers, threads)
- Dramatiq with Redis: 4,800 jobs/sec (10 workers, threads)
- Temporal with PostgreSQL: 8-16 workflows/sec (4 vCores)
- Currant with PostgreSQL: 200 jobs/sec (5 workers) ← **Current state**
...
The Redis backend design provides a clear path to 10-20x performance improvement for high-throughput use cases while maintaining PostgreSQL as the reliable default. The per-job backend selection gives users fine-grained control over the speed/durability tradeoff, making Currant suitable for both critical financial transactions and high-volume analytics workloads.
