...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="postgresql://localhost/workflows",
    ...
    worker_max_concurrent: int = Field(
        default=10,
        description="Maximum concurrent task executions per worker"
    )

    worker_verbose: bool = Field(
        default=False,
    ...
    default_retries: int = Field(
        default=3,
        description="Default number of retry attempts for failed tasks"
    )

    default_retry_backoff_base: float = Field(
        default=2.0,
        description="Base multiplier for exponential backoff between retries"
    )
